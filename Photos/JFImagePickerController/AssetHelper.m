    ...
    [alGroup enumerateAssetsUsingBlock:^(ALAsset *alPhoto, NSUInteger index, BOOL *stop) {
        ...
        if(alPhoto == nil) {
            if (weakSelf.bReverse) {
				if (weakSelf.defaultAssets) {
					[weakSelf.assetPhotos addObjectsFromArray:_defaultAssets];
				}

				[weakSelf.defaultAssets addObject:@"camera"];
