...
@interface PhotoBrowserViewController () <UIScrollViewDelegate, PhotoDelegate>
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [(ImagePickerController *)self.navigationController setLeftTitle:@"取消"];
...
- (void)viewWillDisappear:(BOOL)animated {
	...
	if (!isBrowser) {
		if (ASSETHELPER.selectdPhotos.count>0) {
			[(ImagePickerController *)self.navigationController setLeftTitle:@"预览"];
		} else {
			[(ImagePickerController *)self.navigationController setLeftTitle:@""];
		}
		if (isPreview) {
	...
	if (!isPreview) {
		if ([placeholder.backgroundColor isEqual:[[UIColor blackColor] colorWithAlphaComponent:.1]]) {
			if (ASSETHELPER.selectdPhotos.count>= ASSETHELPER.maxCount) {
                UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:[NSString stringWithFormat:@"最多可以选择%@张照片",@(ASSETHELPER.maxCount)] preferredStyle:UIAlertControllerStyleAlert];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    PhotoView *photoView = (PhotoView *)[photosView viewWithTag:page+2];
	if (photoView!=nil) {
		[photoView reset];
	}
    PhotoView *photoView1 = (PhotoView *)[photosView viewWithTag:page];
	if (photoView1!=nil) {
        [photoView reset];
	}
}
...
- (void)clearScrollViewWithPage:(NSInteger)page{
    ...
    if (page >= photoCount)
        return;
    PhotoView *photoView = (PhotoView *)[photosView viewWithTag:page+1];
	if (photoView) {
...
- (void)loadScrollViewWithPage:(NSInteger)page
{
    ...
    PhotoView *photoView = (PhotoView *)[photosView viewWithTag:page+1];
	if (photoView==nil) {
		photoView = [[PhotoView alloc] initWithFrame:CGRectMake(photosView.frame.size.width*page, 0, photosView.frame.size.width-10, photosView.frame.size.height)];
...
- (void)tap{
    if (self.navigationController.navigationBarHidden) {
        ...
        [UIView animateWithDuration:.3 animations:^{
            CGRect frame = [(ImagePickerController *)self.navigationController customToolbar].frame;
            frame.origin.y = [UIScreen mainScreen].bounds.size.height-44;
            [(ImagePickerController *)self.navigationController customToolbar].frame = frame;
        }];
    } else {
