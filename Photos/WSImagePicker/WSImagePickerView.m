...
- (instancetype)init {
    if(self = [super init]) {
        ...
        _photosMaxCount = 10000;
        _selectIMG = [UIImage imageNamed:@"shangchuan"];
    }
    return self;
}
...
- (void)refreshCollectionView {
    ...
    if(self.viewHeightChanged) {
        self.viewHeightChanged(height,_photosArray);
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row < _photosArray.count) {
        ...
        imgView.image = nil;
//        imgView.image = [UIImage imageNamed:@"shangchuan"];
        imgView.image = _config.selectIMG;
    }
    return cell;
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    switch (buttonIndex) {
        ...
        case 1: {
            ...
            [ImagePickerController setMaxCount:count];
            ImagePickerController *picker = [[ImagePickerController alloc] initWithRootViewController:[UIViewController new]];
            picker.pickerDelegate = self;
            picker.arr = _photosArray;
...
- (void)imagePickerDidFinished:(ImagePickerController *)picker{
    ...
    for (ALAsset *asset in picker.assets) {
        [[ImageManager sharedManager] imageWithAsset:asset resultHandler:^(CGImageRef imageRef, BOOL longImage) {
