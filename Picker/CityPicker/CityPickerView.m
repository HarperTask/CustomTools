...
- (void)selectedOk {
    ...
    if (self.isSaveHistory) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:dicHistory forKey:@"CityPickerView"];
    }else {
        [[NSUserDefaults standardUserDefaults] setObject:nil forKey:@"CityPickerView"];
    }
    
    if ([self.delegate respondsToSelector:@selector(cityPickerView:province:city:area:)]) {
        [self.delegate cityPickerView:self province:self.province city:self.city area:self.area];
        self.delegate = nil;
    }
    [super selectedOk];
}
...
- (void)setSaveHistory:(BOOL)saveHistory {
    ...
    if (saveHistory) {
        NSDictionary *dicHistory = [[NSUserDefaults standardUserDefaults] dictionaryForKey:@"CityPickerView"];
