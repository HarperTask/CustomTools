...
- (void)setupDefault
{
    ...
    _heightPicker      = AUTO_SCALE_H(240); //240
...
- (void)layoutSubviews
{
    ...
    if (self.contentMode == STPickerContentModeBottom) {
        ...
        self.buttonLeft.st_y = self.lineViewDown.st_bottom + CTMarginSmall;
        self.buttonRight.st_y = self.lineViewDown.st_bottom + CTMarginSmall;
    }
}
...
- (void)show
{
    ...
    if (self.contentMode == STPickerContentModeBottom) {
        ...
        CGRect frameContent =  self.contentView.frame;
        frameContent.origin.y -= (STScreenHeight+self.contentView.st_height)/2.0;
        [UIView animateWithDuration:0.3 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            [self.layer setOpacity:1.0];
            self.contentView.frame = frameContent;
...
- (void)setContentMode:(STPickerContentMode)contentMode
{
    ...
    if (contentMode == STPickerContentModeCenter) {
        self.contentView.st_height += CTControlSystemHeight;
    }
}
...
- (UIView *)lineView
{
    if (!_lineView) {
        CGFloat lineX = 0;
        CGFloat lineY = CTControlSystemHeight;
...
//        CGFloat pickerH = self.contentView.st_height - self.lineView.st_bottom;
        CGFloat pickerH = AUTO_SCALE_H(180);
        CGFloat pickerX = 0;
...
{
    if (!_buttonLeft) {
        CGFloat leftW = CTButtonSystemHeight;
        CGFloat leftH = self.lineView.st_top - CTMargin;
        CGFloat leftX = CTMarginBig;
        CGFloat leftY = (self.lineView.st_top - leftH) / 2.0;
...
{
    if (!_labelTitle) {
        CGFloat titleX = self.buttonLeft.st_right + CTMarginSmall;
        CGFloat titleY = 0;
